...
@if (Auth::user()->isAdmin())
    <div class="row">
      <div class="col-12">
        <form class="form-horizontal mt-4" method="post" action="{{route('panevezysphoto-store')}}" enctype="multipart/form-data">
          {!!csrf_field() !!}
          ...
          <div class="form-group">
            <input type="hidden" name="panevezys_id" value="{{$utena['id']}}">
          </div>
          <button type="submit" class="btn btn-success btn-sm float-left">
            Saugoti
          </button>
        </form>
        <a href="{{route('panevezys')}}" class="btn btn-primary btn-sm float-left ml-2">Grįžti</a>
      </div>
    </div>
  @endif
  @endif
